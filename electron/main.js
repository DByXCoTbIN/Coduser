...
function saveState(state) {
  try { fs.mkdirSync(path.dirname(STATE_FILE), { recursive: true }); fs.writeFileSync(STATE_FILE, JSON.stringify(state, null, 2)); } catch {}
}

let appState = loadState();

// ===== Language Detection & Runner =====
const LANG_MAP = {
...
// ===== Folder Scanning =====
const INDEX_FILES = ['index.html', 'index.php', 'index.htm', 'index.js', 'index.ts', 'index.py', 'app.js', 'server.js', 'main.py', 'manage.py'];
const PROJECT_MARKERS = { 'composer.json': 'php', 'package.json': 'node', 'requirements.txt': 'python', 'Gemfile': 'ruby', 'go.mod': 'go', 'Cargo.toml': 'rust', 'pom.xml': 'java' };
...
function scanFolder(dirPath) {
  try {
    ...
    for (const idx of INDEX_FILES) {
      if (files.includes(idx)) { indexFile = idx; break; }
    }

    // Detect project type from config files
    let projectType = null;
    for (const [marker, type] of Object.entries(PROJECT_MARKERS)) {
      if (files.includes(marker)) { projectType = type; break; }
    }
    ...
    let serverType = null;
    if (projectType === 'php' || dominantLang === 'php' || files.some(f => f.endsWith('.php'))) serverType = 'php';
    else if (projectType === 'node' || files.includes('server.js') || files.includes('app.js')) serverType = 'node';
    else if (projectType === 'python' || files.some(f => f.endsWith('.py'))) serverType = 'python';
...
function buildPreviewUrl(folderInfo, port) {
  ...
  if (folderInfo.serverType === 'php' || folderInfo.serverType === 'node' || folderInfo.serverType === 'python') {
...
function isPortUsed(port) {
  try {
    const r = execSync(`ss -tlnp sport = :${port} 2>/dev/null || netstat -tlnp 2>/dev/null | grep :${port}`, { timeout: 3000, encoding: 'utf-8', stdio: ['pipe', 'pipe', 'ignore'] });
...
function startPhpDevServer(port = 8080, dir = WWW_DIR) {
  ...
  try {
    ...
    phpProcess = spawn(php, ['-S', `0.0.0.0:${port}`, '-t', dir], {
      detached: true, stdio: 'ignore',
      env: { ...process.env, PHP_CLI_SERVER_WORKERS: '4' }
    });
    phpProcess.unref();
    phpProcess.on('error', (e) => { console.error('PHP server error:', e.message); phpProcess = null; });
    phpProcess.on('exit', (code) => { console.log('PHP server exited with code', code); phpProcess = null; });
...
function findNode() {
  try { execSync('node --version', { timeout: 3000, stdio: 'ignore' }); return 'node'; } catch {}
  return null;
}
...
function isServiceInstalled(name) {
  try {
    if (process.platform === 'linux') {
      ...
      try { execSync(`which ${name}`, { timeout: 3000, stdio: 'ignore' }); return true; } catch {}
      // Check package manager
      try { execSync(`pacman -Qi ${name} 2>/dev/null`, { timeout: 5000, stdio: 'ignore' }); return true; } catch {}
      try { execSync(`dpkg -l ${name} 2>/dev/null`, { timeout: 5000, stdio: 'ignore' }); return true; } catch {}
      try { execSync(`rpm -q ${name} 2>/dev/null`, { timeout: 5000, stdio: 'ignore' }); return true; } catch {}
      return false;
    }
    ...
    if (process.platform === 'win32') {
      ...
      const paths = ['C:\\xampp\\mysql\\bin\\mysqld.exe', 'C:\\Program Files\\MySQL', 'C:\\Program Files (x86)\\MySQL'];
...
function checkServers() {
  ...
  for (const name of mysqlBinaries) {
    if (isProcessRunning(name)) { mysql = true; break; }
  }
  if (!mysql) mysql = isPortUsed('3306');
  for (const name of mysqlServices) {
    if (isServiceInstalled(name)) { mysqlInstalled = true; break; }
  }
  // Also check if the systemd service exists
  if (!mysqlInstalled) {
    try {
      execSync('systemctl list-unit-files mariadb.service 2>/dev/null | grep -q mariadb', { timeout: 3000, stdio: 'ignore' });
      mysqlInstalled = true;
    } catch {}
  }
  ...
  for (const name of apacheBinaries) {
    if (isProcessRunning(name)) { apache = true; break; }
  }
  if (!apache) apache = isPortUsed('80') || isPortUsed('443');
  for (const name of apacheServices) {
    if (isServiceInstalled(name)) { apacheInstalled = true; break; }
  }
  // Also check if the systemd service exists
  if (!apacheInstalled) {
    try {
      execSync('systemctl list-unit-files httpd.service 2>/dev/null | grep -q httpd', { timeout: 3000, stdio: 'ignore' });
...
function startService(name) {
  ...
  try {
    if (process.platform === 'linux') {
      try { execSync(`sudo systemctl start ${service}`, { timeout: 10000, stdio: 'pipe' }); return { success: true }; } catch (e) {
        // Try without sudo
        try { execSync(`systemctl --user start ${service}`, { timeout: 10000, stdio: 'pipe' }); return { success: true }; } catch {}
        return { success: false, error: `Could not start ${service}. May require sudo.` };
      }
    }
    if (process.platform === 'darwin') {
      execSync(`brew services start ${service} 2>/dev/null || sudo apachectl start`, { timeout: 10000, stdio: 'pipe' });
      return { success: true };
    }
...
function stopService(name) {
  ...
  try {
    if (process.platform === 'linux') {
      try { execSync(`sudo systemctl stop ${service}`, { timeout: 10000, stdio: 'pipe' }); return { success: true }; } catch (e) {
        try { execSync(`systemctl --user stop ${service}`, { timeout: 10000, stdio: 'pipe' }); return { success: true }; } catch {}
        return { success: false, error: `Could not stop ${service}. May require sudo.` };
      }
    }
    if (process.platform === 'darwin') {
      execSync(`brew services stop ${service} 2>/dev/null || sudo apachectl stop`, { timeout: 10000, stdio: 'pipe' });
      return { success: true };
    }
    return { success: false, error: 'Service management not supported on this platform' };
  } catch (e) { return { success: false, error: e.message }; }
...
function createWindow() {
  mainWindow = new BrowserWindow({
    width: 1600, height: 900, minWidth: 1000, minHeight: 600,
    title: 'Brauser IDE', frame: false, titleBarStyle: 'hidden',
    backgroundColor: '#1e1e1e',
    webPreferences: { nodeIntegration: false, contextIsolation: true, preload: path.join(__dirname, 'preload.js'), sandbox: false, webSecurity: false },
    show: false
  });

  mainWindow.loadFile(path.join(__dirname, '..', 'ide', 'index.html'));
  mainWindow.once('ready-to-show', () => mainWindow.show());
  mainWindow.on('closed', () => { stopPhpDevServer(); mainWindow = null; });
...
function send(channel, ...args) { mainWindow?.webContents.send(channel, ...args); }
...
function setupIPC() {
  ...
  // File System
  ipcMain.handle('fs:read', (_, p) => { try { return { success: true, content: fs.readFileSync(p, 'utf-8') }; } catch (e) { return { success: false, error: e.message }; } });
  ipcMain.handle('fs:write', (_, p, c) => { try { const d = path.dirname(p); if (!fs.existsSync(d)) fs.mkdirSync(d, { recursive: true }); fs.writeFileSync(p, c, 'utf-8'); return { success: true }; } catch (e) { return { success: false, error: e.message }; } });
  ipcMain.handle('fs:list', (_, p) => {
    try {
      if (!fs.existsSync(p)) return { success: false, error: 'Not found' };
      const items = fs.readdirSync(p, { withFileTypes: true }).filter(i => !i.name.startsWith('.') && i.name !== 'node_modules').map(i => ({ name: i.name, path: path.join(p, i.name), isDir: i.isDirectory(), size: i.isFile() ? fs.statSync(path.join(p, i.name)).size : 0 }));
      return { success: true, items };
    } catch (e) { return { success: false, error: e.message }; }
  });
  ipcMain.handle('fs:mkdir', (_, p) => { try { fs.mkdirSync(p, { recursive: true }); return { success: true }; } catch (e) { return { success: false, error: e.message }; } });
  ipcMain.handle('fs:delete', (_, p) => { try { if (fs.statSync(p).isDirectory()) fs.rmSync(p, { recursive: true }); else fs.unlinkSync(p); return { success: true }; } catch (e) { return { success: false, error: e.message }; } });
  ipcMain.handle('fs:rename', (_, o, n) => { try { fs.renameSync(o, n); return { success: true }; } catch (e) { return { success: false, error: e.message }; } });

  // Dialogs
  ipcMain.handle('dialog:open-file', async () => { const r = await dialog.showOpenDialog(mainWindow, { properties: ['openFile'] }); return r.canceled ? null : r.filePaths[0]; });
  ipcMain.handle('dialog:open-folder', async () => { const r = await dialog.showOpenDialog(mainWindow, { properties: ['openDirectory'] }); return r.canceled ? null : r.filePaths[0]; });

  // Servers
  ipcMain.handle('servers:status', () => checkServers());
  ipcMain.handle('servers:start-php', (_, port, dir) => startPhpDevServer(port, dir || WWW_DIR));
  ipcMain.handle('servers:stop-php', () => { stopPhpDevServer(); return { success: true }; });
  ...
  // State
  ipcMain.handle('state:save', (_, data) => { appState = { ...appState, ...data }; saveState(appState); return { success: true }; });
  ...
  // Git
  ipcMain.handle('git:status', (_, cwd) => { try { return { success: true, output: execSync('git status --porcelain', { cwd, encoding: 'utf-8', timeout: 5000 }) }; } catch (e) { return { success: false, error: e.message }; } });
  ipcMain.handle('git:branch', (_, cwd) => { try { return { success: true, branch: execSync('git branch --show-current', { cwd, encoding: 'utf-8', timeout: 5000 }).trim() }; } catch (e) { return { success: false }; } });
  ipcMain.handle('git:commit', (_, cwd, msg) => { try { execSync('git add .', { cwd, timeout: 10000 }); return { success: true, output: execSync(`git commit -m "${msg.replace(/"/g, '\\"')}"`, { cwd, encoding: 'utf-8', timeout: 10000 }) }; } catch (e) { return { success: false, error: e.message }; } });
  ipcMain.handle('git:init', (_, cwd) => { try { return { success: true, output: execSync('git init', { cwd, encoding: 'utf-8', timeout: 5000 }) }; } catch (e) { return { success: false }; } });

  // Shell
  ipcMain.handle('shell:open', (_, url) => { shell.openExternal(url); return { success: true }; });
  ...
  ipcMain.handle('term:run', (_, cmd, cwd) => {
    return new Promise(resolve => {
      const isLongRunning = /\b(php\s+-S|python\s+-m\s+http|node\s+server|npm\s+run\s+dev|npm\s+start|yarn\s+dev|watch|serve|forever|pm2)\b/.test(cmd);
  ...
  ipcMain.handle('term:kill', (_, pid) => {
    try { process.kill(pid, 'SIGTERM'); return { success: true }; } catch { return { success: false }; }
  });
